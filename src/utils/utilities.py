...
def render_text_with_shadow(offset, text, colour=(255, 255, 255), drop_colour=(128, 128, 128), font=default_font):
    # make the drop-shadow
    text_bitmap = font.render(text, True, drop_colour)

    rendered_text = pg.Surface((text_bitmap.get_width()+offset, text_bitmap.get_height()+offset), flags=pg.SRCALPHA)

    rendered_text.blit(text_bitmap, (offset, offset))
    # make the overlay text
    text_bitmap = font.render(text, True, colour)
...
def calculate_shoot_angle(shooter, projectile_speed, target):
    ...
    x = asin(max(-1, min(1, v2.length()/v1 * sin(alpha))))
